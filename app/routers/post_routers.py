...
from app.schemas.post_schemas import PostIn, SuccessAction, PostOut, Pagination, CreatePostOut, FieldsToUpdate
from app.services.post_service import (
    create_post_service, 
    get_all_post_service, 
    get_post_by_id_service, 
    get_post_by_title_service, 
    delete_post_service, 
    get_post_by_author_service,
...
@post_router.post("/posts", response_model=CreatePostOut)
...
@post_router.get("/posts/by-post_id", response_model=PostOut)
...
@post_router.get("/posts/by-author", response_model=List[PostOut])
def get_post_by_author(author:str):
    try:
        return get_post_by_author_service(author)
    ...
    #     raise HTTPException(
    #     status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    #     detail="UNEXEPTION_ERROR_OCCURED"
    # )
...
@post_router.put("/posts/{post_id}", response_model=CreatePostOut)
def update_post(user_id: Annotated[str, Depends(get_current_user)], to_update: FieldsToUpdate):
    try:
        feedback = update_post_service(user_id, to_update)
